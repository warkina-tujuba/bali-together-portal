  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-background/95 pb-[env(safe-area-inset-bottom)] backdrop-blur">
      <div className="mx-auto flex max-w-2xl items-center justify-around px-2 py-1.5">
        {items.map((i) => (
          <Link key={i.to} to={i.to} className="flex-1 rounded-2xl py-2.5 text-center text-sm font-medium text-muted-foreground transition [&.active]:text-primary" activeProps={{ className: "active" }}>
            {i.label}
          </Link>
        ))}
      </div>
